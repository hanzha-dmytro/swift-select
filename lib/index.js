class SwiftSelect {
    ...
    #selectDefaultOption() {
        if (!this.#options?.length || !this.#settings.withDefaultOption) return;

        const selectedValue = this.#options[0]?.value ?? null;
        this.#selectOption(selectedValue, true);
    }

    #validateSelectedOptions(){
        this.#selected = this.#selected.filter((option) => this.#isOptionAvailable(option));
